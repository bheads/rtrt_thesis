 ...
 *  \return Returns a constant vec4 object that is the results of lhs + rhs.
 */
inline const vec4 operator+( const vec4 &lhs, float rhs )
{
    vec4 ret;
    asm(    "movaps %1, %%xmm0 \n\t"
            "movss %2, %%xmm1 \n\t"
 ...
 *  \return Returns a constant vec4 object that is the results of lhs + rhs.
 */
inline const vec4 operator+( float lhs, const vec4 &rhs )
{
    vec4 ret;
    asm(    "movaps %2, %%xmm1 \n\t"
            "movss %1, %%xmm0 \n\t"
 ...
 */
inline const vec4 operator+( const vec4 &o )
{
    return( vec4( o ));
}
 ...
 */
inline const vec4 cross( const vec4 &a, const vec4 &b )
{
    /*
 ...
 */
inline vec4 &cross( vec4 &dest, const vec4 &a, const vec4 &b )
{
...
*/
inline const color clamped( const color &c, const float min, const float max )
{
    color ret;
    asm(    "movaps %1, %%xmm0 \n\t"
            "movss %2, %%xmm1 \n\t"
...
/**
  \brief Clamp a copied color to min and max values
    ...
    \returns refrence to copied clamped color
*/
inline color &clamp( color &c, const float min = 0.0f, const float max = 1.0f )
{
    asm(    "movaps %1, %%xmm0 \n\t"
            "movss %2, %%xmm1 \n\t"
            "shufps $0x00, %%xmm1, %%xmm1 \n\t"
